...
static TSS2_RC
TPM2_GENERATED_Unmarshal(uint8_t const   buffer[],
                         size_t          buffer_size,
                         size_t         *offset,
...
#define TPMS_PCR_MARSHAL(type, firstFieldMarshal)                                                  \
    TSS2_RC                                                                                        \
    Tss2_MU_##type##_Marshal(const type *src, uint8_t buffer[], size_t buffer_size,                \
                             size_t *offset) {                                                     \
        ...
        LOG_DEBUG("Marshalling " #type " from 0x%" PRIxPTR " to buffer 0x%" PRIxPTR                \
                  " at index 0x%zx",                                                               \
                  (uintptr_t) & src, (uintptr_t)buffer, offset ? *offset : 0xffff);                \
                                                                                                   \
        ...
        for (i = 0; i < src->sizeofSelect; i++) {                                                  \
            ret = Tss2_MU_BYTE_Marshal(src->pcrSelect[i], buffer, buffer_size, &local_offset);     \
            if (ret != TSS2_RC_SUCCESS)                                                            \
                return ret;                                                                        \
        }                                                                                          \
                                                                                                   \
...
#define TPMS_PCR_UNMARSHAL(type, firstFieldUnmarshal)                                              \
    TSS2_RC                                                                                        \
    Tss2_MU_##type##_Unmarshal(uint8_t const buffer[], size_t buffer_size, size_t *offset,         \
                               type *dest) {                                                       \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
        size_t  local_offset = 0;                                                                  \
        size_t  i;                                                                                 \
        ...
        LOG_DEBUG("Unmarshaling " #type " from 0x%" PRIxPTR " to buffer 0x%" PRIxPTR               \
                  " at index 0x%zx",                                                               \
                  (uintptr_t)dest, (uintptr_t)buffer, offset ? *offset : 0xffff);                  \
                                                                                                   \
        if (offset) {                                                                              \
            local_offset = *offset;                                                                \
        } else if (!dest) {                                                                        \
...
TPMS_PCR_UNMARSHAL(
    TPMS_PCR_SELECTION,
    Tss2_MU_TPMI_ALG_HASH_Unmarshal(buffer, buffer_size, &local_offset, dest ? &dest->hash : NULL))

TPMS_PCR_UNMARSHAL(
    TPMS_TAGGED_PCR_SELECT,
    Tss2_MU_UINT32_Unmarshal(buffer, buffer_size, &local_offset, dest ? &dest->tag : NULL))
...
#define TPMS_MARSHAL_0(type)                                                                       \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
        ...
        LOG_DEBUG("Marshalling " #type " from 0x%" PRIxPTR " to buffer 0x%" PRIxPTR                \
                  " at index 0x%zx",                                                               \
                  (uintptr_t) & src, (uintptr_t)buffer, *offset);                                  \
                                                                                                   \
        return TSS2_RC_SUCCESS;                                                                    \
    }
...
#define TPMS_UNMARSHAL_2_U(type, m1, fn1, m2, fn2)                                                 \
    TSS2_RC Tss2_MU_##type##_Unmarshal(uint8_t const buffer[], size_t buffer_size, size_t *offset, \
        ...
        if (ret != TSS2_RC_SUCCESS)                                                                \
            return ret;                                                                            \
                                                                                                   \
        ret = fn2(buffer, buffer_size, &local_offset, dest ? dest->m1 : tmp_dest.m1,               \
                  dest ? &dest->m2 : NULL);                                                        \
                                                                                                   \
        if (offset && ret == TSS2_RC_SUCCESS) {                                                    \
            *offset = local_offset;                                                                \
...
#define TPMS_MARSHAL_5(type, m1, op1, fn1, m2, op2, fn2, m3, op3, fn3, m4, op4, fn4, m5, op5, fn5) \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
                                     size_t *offset) {                                             \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
        size_t  local_offset = 0;                                                                  \
...
#define TPMS_MARSHAL_7(type, m1, op1, fn1, m2, op2, fn2, m3, op3, fn3, m4, op4, fn4, m5, op5, fn5, \
                       m6, op6, fn6, m7, op7, fn7)                                                 \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
                                     size_t *offset) {                                             \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
        size_t  local_offset = 0;                                                                  \
...
#define TPMS_MARSHAL_7_U(type, m1, op1, fn1, m2, op2, fn2, m3, op3, fn3, m4, op4, fn4, m5, op5,    \
                         fn5, m6, op6, fn6, m7, op7, fn7)                                          \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
                                     size_t *offset) {                                             \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
        size_t  local_offset = 0;                                                                  \
...
#define TPMS_UNMARSHAL_7_U(type, m1, fn1, m2, fn2, m3, fn3, m4, fn4, m5, fn5, m6, fn6, m7, fn7)    \
    TSS2_RC Tss2_MU_##type##_Unmarshal(uint8_t const buffer[], size_t buffer_size, size_t *offset, \
        ...
        if (ret != TSS2_RC_SUCCESS)                                                                \
            return ret;                                                                            \
                                                                                                   \
        ret = fn7(buffer, buffer_size, &local_offset, dest ? dest->m2 : 0,                         \
                  dest ? &dest->m7 : NULL);                                                        \
                                                                                                   \
        if (offset && ret == TSS2_RC_SUCCESS) {                                                    \
            *offset = local_offset;                                                                \
...
#define TPMS_MARSHAL_11(type, m1, op1, fn1, m2, op2, fn2, m3, op3, fn3, m4, op4, fn4, m5, op5,     \
                        fn5, m6, op6, fn6, m7, op7, fn7, m8, op8, fn8, m9, op9, fn9, m10, op10,    \
                        fn10, m11, op11, fn11)                                                     \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
                                     size_t *offset) {                                             \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
...
// NOLINTBEGIN(bugprone-macro-parentheses)
#define TPMS_UNMARSHAL_11(type, m1, fn1, m2, fn2, m3, fn3, m4, fn4, m5, fn5, m6, fn6, m7, fn7, m8, \
                          fn8, m9, fn9, m10, fn10, m11, fn11)                                      \
    TSS2_RC Tss2_MU_##type##_Unmarshal(uint8_t const buffer[], size_t buffer_size, size_t *offset, \
                                       type *dest) {                                               \
        TSS2_RC ret = TSS2_RC_SUCCESS;                                                             \
        size_t  local_offset = 0;                                                                  \
...
TPMS_UNMARSHAL_2(TPMS_SCHEME_XOR, hashAlg, Tss2_MU_UINT16_Unmarshal, kdf, Tss2_MU_UINT16_Unmarshal)
...
TPMS_MARSHAL_1(TPMS_KEYEDHASH_PARMS, scheme, ADDR, Tss2_MU_TPMT_KEYEDHASH_SCHEME_Marshal)

TPMS_UNMARSHAL_1(TPMS_KEYEDHASH_PARMS, scheme, Tss2_MU_TPMT_KEYEDHASH_SCHEME_Unmarshal)
