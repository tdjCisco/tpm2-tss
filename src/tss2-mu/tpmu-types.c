...
static TSS2_RC
marshal_tab(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset, size_t size) {
    ...
    if (buffer == NULL && offset == NULL) {
        ...
        *offset += size;
        ...
        LOG_DEBUG("buffer_size: %zu with offset: %zu are insufficient for "
                  "object of size %zu",
                  buffer_size, local_offset, size);
        return TSS2_MU_RC_INSUFFICIENT_BUFFER;
    }

    LOG_DEBUG("Marshalling TPMU tab of %d bytes from 0x%" PRIxPTR " to buffer 0x%" PRIxPTR
              " at index 0x%zx",
              (int)size, (uintptr_t)src, (uintptr_t)buffer, local_offset);
...
static TSS2_RC
marshal_hash_sha(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_SHA1_DIGEST_SIZE);
}

static TSS2_RC
marshal_hash_sha256(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_SHA256_DIGEST_SIZE);
}

static TSS2_RC
marshal_hash_sha384(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_SHA384_DIGEST_SIZE);
}

static TSS2_RC
marshal_hash_sha512(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_SHA512_DIGEST_SIZE);
}

static TSS2_RC
marshal_sm3_256(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_SM3_256_DIGEST_SIZE);
}

static TSS2_RC
marshal_ecc(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, sizeof(TPMS_ECC_POINT));
}

static TSS2_RC
marshal_rsa(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, TPM2_MAX_RSA_KEY_BYTES);
}

static TSS2_RC
marshal_symmetric(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, sizeof(TPM2B_DIGEST));
}

static TSS2_RC
marshal_keyedhash(BYTE const *src, uint8_t buffer[], size_t buffer_size, size_t *offset) {
    return marshal_tab(src, buffer, buffer_size, offset, sizeof(TPM2B_DIGEST));
}
...
static TSS2_RC
unmarshal_tab(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest, size_t size) {
    size_t local_offset = 0;
    ...
    LOG_DEBUG("Marshalling TPMU tab of %d bytes from buffer 0x%" PRIxPTR
              " at index 0x%zx to dest 0x%" PRIxPTR,
              (int)size, (uintptr_t)buffer, local_offset, (uintptr_t)dest);
...
static TSS2_RC
unmarshal_hash_sha(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_SHA1_DIGEST_SIZE);
}

static TSS2_RC
unmarshal_hash_sha256(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_SHA256_DIGEST_SIZE);
}

static TSS2_RC
unmarshal_hash_sha384(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_SHA384_DIGEST_SIZE);
}

static TSS2_RC
unmarshal_hash_sha512(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_SHA512_DIGEST_SIZE);
}

static TSS2_RC
unmarshal_sm3_256(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_SM3_256_DIGEST_SIZE);
}

static TSS2_RC
unmarshal_ecc(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, sizeof(TPMS_ECC_POINT));
}

static TSS2_RC
unmarshal_rsa(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, TPM2_MAX_RSA_KEY_BYTES);
}

static TSS2_RC
unmarshal_symmetric(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, sizeof(TPM2B_DIGEST));
}

static TSS2_RC
unmarshal_keyedhash(uint8_t const buffer[], size_t buffer_size, size_t *offset, BYTE *dest) {
    return unmarshal_tab(buffer, buffer_size, offset, dest, sizeof(TPM2B_DIGEST));
}
 ...
 *         marshaling no-op.
 */
#define TPMU_MARSHAL(type, sel, op, m, fn, sel2, op2, m2, fn2, sel3, op3, m3, fn3, sel4, op4, m4,  \
                     fn4, sel5, op5, m5, fn5, sel6, op6, m6, fn6, sel7, op7, m7, fn7, sel8, op8,   \
                     m8, fn8, sel9, op9, m9, fn9, sel10, op10, m10, fn10, sel11, op11, m11, fn11,  \
                     sel12, op12, m12, fn12, ...)                                                  \
...
#define TPMU_MARSHAL2(type, sel, op, m, fn, ...)                                                   \
    TPMU_MARSHAL(type, sel, op, m, fn, __VA_ARGS__, -1, ADDR, m, marshal_null, -2, ADDR, m,        \
                 marshal_null, -3, ADDR, m, marshal_null, -4, ADDR, m, marshal_null, -5, ADDR, m,  \
                 marshal_null, -6, ADDR, m, marshal_null, -7, ADDR, m, marshal_null, -8, ADDR, m,  \
                 marshal_null, -9, ADDR, m, marshal_null, -10, ADDR, m, marshal_null)
...
#define TPMU_UNMARSHAL(type, sel, m, fn, sel2, m2, fn2, sel3, m3, fn3, sel4, m4, fn4, sel5, m5,    \
                       fn5, sel6, m6, fn6, sel7, m7, fn7, sel8, m8, fn8, sel9, m9, fn9, sel10,     \
                       m10, fn10, sel11, m11, fn11, sel12, m12, fn12, ...)                         \
...
#define TPMU_UNMARSHAL2(type, sel, m, fn, ...)                                                     \
    TPMU_UNMARSHAL(type, sel, m, fn, __VA_ARGS__, -1, m, unmarshal_null, -2, m, unmarshal_null,    \
                   -3, m, unmarshal_null, -4, m, unmarshal_null, -5, m, unmarshal_null, -6, m,     \
                   unmarshal_null, -7, m, unmarshal_null, -8, m, unmarshal_null, -9, m,            \
                   unmarshal_null, -10, m, unmarshal_null)
...
TPMU_UNMARSHAL2(TPMU_HA,
                ...
                TPM2_ALG_SHA256,
                sha256[0],
                unmarshal_hash_sha256,
                TPM2_ALG_SHA384,
                sha384[0],
                unmarshal_hash_sha384,
                TPM2_ALG_SHA512,
                sha512[0],
                unmarshal_hash_sha512,
                TPM2_ALG_SM3_256,
                sm3_256[0],
                unmarshal_sm3_256)
...
TPMU_MARSHAL2(TPMU_PUBLIC_PARMS,
              ...
              ADDR,
              mlkemDetail,
              Tss2_MU_TPMS_MLKEM_PARMS_Marshal,
              TPM2_ALG_MLDSA,
              ADDR,
              mldsaDetail,
              Tss2_MU_TPMS_MLDSA_PARMS_Marshal,
              TPM2_ALG_HASH_MLDSA,
              ADDR,
              hashMldsaDetail,
              Tss2_MU_TPMS_HASH_MLDSA_PARMS_Marshal)
TPMU_UNMARSHAL2(TPMU_PUBLIC_PARMS,
                ...
                TPM2_ALG_MLKEM,
                mlkemDetail,
                Tss2_MU_TPMS_MLKEM_PARMS_Unmarshal,
                TPM2_ALG_MLDSA,
                mldsaDetail,
                Tss2_MU_TPMS_MLDSA_PARMS_Unmarshal,
                TPM2_ALG_HASH_MLDSA,
                hashMldsaDetail,
                Tss2_MU_TPMS_HASH_MLDSA_PARMS_Unmarshal)
...
TPMU_MARSHAL2(TPMU_NAME,
              ...
              sizeof(TPM2_ALG_ID) + TPM2_SHA1_DIGEST_SIZE,
              ADDR,
              digest,
              Tss2_MU_TPMT_HA_Marshal,
