...
TSS2_RC
Tss2_Sys_VerifyDigestSignature_Prepare(TSS2_SYS_CONTEXT          *sysContext,
                                       ...
                                       const TPMT_SIGNATURE      *signature)
{
    ...
    if (!ctx || !context || !signature)
        return TSS2_SYS_RC_BAD_REFERENCE;

    rval = CommonPreparePrologue(ctx, TPM2_CC_VerifyDigestSignature);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_VerifyDigestSignature(TSS2_SYS_CONTEXT             *sysContext,
                               TPMI_DH_OBJECT                keyHandle,
                               ...
                               TSS2L_SYS_AUTH_RESPONSE      *rspAuthsArray)
{
    ...
    return Tss2_Sys_VerifyDigestSignature_Complete(sysContext, validation);
}
