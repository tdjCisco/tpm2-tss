                                      ...
                                      const TPM2B_MAX_BUFFER  *buffer)
{
    ...
    rval = CommonPreparePrologue(ctx, TPM2_CC_SignSequenceComplete);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_SignSequenceComplete(TSS2_SYS_CONTEXT             *sysContext,
                              TPMI_DH_OBJECT                sequenceHandle,
                              TPMI_DH_OBJECT                keyHandle,
                              ...
                              TSS2L_SYS_AUTH_RESPONSE      *rspAuthsArray)
{
    ...
    rval = Tss2_Sys_SignSequenceComplete_Prepare(sysContext, sequenceHandle, keyHandle, buffer);
    if (rval)
        return rval;
    ...
    return Tss2_Sys_SignSequenceComplete_Complete(sysContext, signature);
}
