...
/** One-Call function for TPM2_VerifyDigestSignature
 *
 * This function invokes the TPM2_VerifyDigestSignature command in a one-call
 ...
 * @param[in]  context The context for the signature.
 * @param[in]  digest The digest that was signed.
 * @param[in]  signature The signature to be verified.
 * @param[out] validation Ticket indicating successful signature validation (callee-allocated).
...
Esys_VerifyDigestSignature(ESYS_CONTEXT                *esysContext,
     ...
     */
    do {
        r = Esys_VerifyDigestSignature_Finish(esysContext, validation);
...
/** Asynchronous function for TPM2_VerifyDigestSignature
 *
 * This function invokes the TPM2_VerifyDigestSignature command in asynchronous variant.
 *
 * @param[in,out] esysContext The ESYS_CONTEXT.
 * @param[in]  keyHandle Handle of the verification key.
 * @param[in]  shandle1 Session handle for authorization of keyHandle.
...
/** Asynchronous finish function for TPM2_VerifyDigestSignature
 *
 * This function returns the results of a TPM2_VerifyDigestSignature command
 * invoked via Esys_VerifyDigestSignature_Async.
 *
 * @param[in,out] esysContext The ESYS_CONTEXT.
 * @param[out] validation Ticket indicating successful signature validation (callee-allocated).
...
Esys_VerifyDigestSignature_Finish(ESYS_CONTEXT *esysContext, TPMT_TK_VERIFIED **validation) {
    ...
    /* Allocate memory for response parameter */
    if (validation != NULL) {
     ...
     */
    r = Tss2_Sys_VerifyDigestSignature_Complete(esysContext->sys,
