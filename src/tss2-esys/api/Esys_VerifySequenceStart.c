...
/*******************************************************************************
 * Copyright 2026, Fraunhofer SIT sponsored by Infineon Technologies AG
 * All rights reserved.
 ******************************************************************************/
 ...
 * @param[in]  keyHandle Handle of the verification key.
 * @param[in]  shandle1 Session handle for authorization of keyHandle.
 * @param[in]  shandle2 Second session handle.
 * @param[in]  shandle3 Third session handle.
 * @param[in]  auth Authorization value for the sequence.
 * @param[in]  hint Signature hint for verification.
 * @param[in]  context The context for the signature sequence.
 * @param[out] sequenceHandle Handle for verification sequence (callee-allocated).
 * @retval TSS2_RC_SUCCESS if the function call was a success.
 * @retval TSS2_ESYS_RC_BAD_REFERENCE if the esysContext or sequenceHandle is NULL.
 * @retval TSS2_ESYS_RC_BAD_CONTEXT: if esysContext corruption is detected.
 * @retval TSS2_ESYS_RC_MEMORY: if the ESAPI cannot allocate enough memory.
 * @retval TSS2_ESYS_RC_BAD_SEQUENCE: if the context has an asynchronous operation already pending.
...
Esys_VerifySequenceStart(ESYS_CONTEXT               *esysContext,
    ...
    r = Esys_VerifySequenceStart_Async(esysContext, keyHandle, shandle1, shandle2, shandle3,
                                        auth, hint, context);
     ...
     */
    do {
        r = Esys_VerifySequenceStart_Finish(esysContext, sequenceHandle);
...
/** Asynchronous function for TPM2_VerifySequenceStart
 *
 * This function invokes the TPM2_VerifySequenceStart command in asynchronous variant.
 *
 * @param[in,out] esysContext The ESYS_CONTEXT.
 * @param[in]  keyHandle Handle of the verification key.
 * @param[in]  shandle1 Session handle for authorization of keyHandle.
...
Esys_VerifySequenceStart_Async(ESYS_CONTEXT               *esysContext,
                               ...
                               const TPM2B_SIGNATURE_CTX  *context) {
    TSS2_RC r;
    LOG_TRACE("context=%p, keyHandle=%" PRIx32 ", auth=%p, hint=%p, context=%p",
              esysContext, keyHandle, auth, hint, context);
    ...
    r = Tss2_Sys_VerifySequenceStart_Prepare(esysContext->sys,
...
/** Asynchronous finish function for TPM2_VerifySequenceStart
 *
 * This function returns the results of a TPM2_VerifySequenceStart command
 * invoked via Esys_VerifySequenceStart_Async.
 *
 * @param[in,out] esysContext The ESYS_CONTEXT.
 * @param[out] sequenceHandle Handle for verification sequence (callee-allocated).
...
Esys_VerifySequenceStart_Finish(ESYS_CONTEXT *esysContext, ESYS_TR *sequenceHandle) {
    ...
    /* Allocate ESYS handle for sequence */
    ...
    *sequenceHandle = esysContext->esys_handle_cnt++;
    ...
    /* Create resource object for the sequence handle */
     ...
     */
    r = Tss2_Sys_VerifySequenceStart_Complete(esysContext->sys, &sequenceHandleNode->rsrc.handle);
